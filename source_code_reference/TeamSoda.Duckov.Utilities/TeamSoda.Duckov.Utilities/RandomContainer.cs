...
namespace Duckov.Utilities
{
	...
	public class RandomContainer<T> : IPercentRefreshable
	{
		...
		public T GetRandom(float lowPercent = 0f)
		{
			...
			float num2 = global::UnityEngine.Random.Range(num * lowPercent, num);
			float num3 = 0f;
		...
		public T GetRandom(global::System.Random overrideRandom, float lowPercent = 0f)
		{
			if (this.entries.Count < 1)
			{
				return default(T);
		...
		public T GetRandom(global::System.Random overrideRandom, Func<T, bool> predicator, float lowPercent = 0f)
		{
			if (this.entries.Count < 1)
			{
				return default(T);
			}
			List<RandomContainer<T>.Entry> list = this.entries.Where((RandomContainer<T>.Entry e) => predicator(e.value)).ToList<RandomContainer<T>.Entry>();
			if (list.Count < 1)
			{
				return default(T);
			}
		...
		public List<T> GetRandomMultiple(int count, bool repeatable = true)
		{
			...
			for (int i = 0; i < count; i++)
			{
				...
				int num;
				T t = RandomContainer<T>.<GetRandomMultiple>g__GetOnceInCandidates|8_1(out num, ref CS$<>8__locals1);
				list.Add(t);
				if (!repeatable)
				{
					CS$<>8__locals1.candidates.RemoveAt(num);
					RandomContainer<T>.<GetRandomMultiple>g__RecalculateTotalWeight|8_0(ref CS$<>8__locals1);
				}
			}
			return list;
		...
		public static RandomContainer<string> FromString(string str)
		{
			...
			foreach (string text in array)
			{
				...
				else
				{
					...
					{
						float num = 1f;
						if (array3.Length == 2 && !float.TryParse(array3[1], out num))
						{
							Debug.LogError("Cannot resolve random container entry:\n " + text + " \n cannot resolve weight");
						}
						else
						{
							randomContainer.AddEntry(text2, num);
						}
					}
				}
			}
		...
		{
			float num = global::UnityEngine.Random.Range(0f, A_1.totalWeight);
			float num2 = 0f;
