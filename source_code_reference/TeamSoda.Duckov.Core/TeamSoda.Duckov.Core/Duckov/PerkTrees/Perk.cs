using System;
using System.Text;
using ItemStatsSystem;
using Sirenix.OdinInspector;
using SodaCraft.Localizations;
using UnityEngine;

namespace Duckov.PerkTrees
{
	// Token: 0x02000247 RID: 583
	public class Perk : MonoBehaviour, ISelfValidator
	{
		// Token: 0x1700032F RID: 815
		// (get) Token: 0x0600121A RID: 4634 RVA: 0x0004500B File Offset: 0x0004320B
		public bool LockInDemo
		{
			get
			{
				return this.lockInDemo;
			}
		}

		// Token: 0x17000330 RID: 816
		// (get) Token: 0x0600121B RID: 4635 RVA: 0x00045013 File Offset: 0x00043213
		public DisplayQuality DisplayQuality
		{
			get
			{
				return this.quality;
			}
		}

		// Token: 0x17000331 RID: 817
		// (get) Token: 0x0600121C RID: 4636 RVA: 0x0004501B File Offset: 0x0004321B
		public Sprite Icon
		{
			get
			{
				return this.icon;
			}
		}

		// Token: 0x17000332 RID: 818
		// (get) Token: 0x0600121E RID: 4638 RVA: 0x00045025 File Offset: 0x00043225
		// (set) Token: 0x0600121D RID: 4637 RVA: 0x00045023 File Offset: 0x00043223
		[LocalizationKey("Perks")]
		private string description
		{
			get
			{
				if (!this.hasDescription)
				{
					return string.Empty;
				}
				return this.displayName + "_Desc";
			}
			set
			{
			}
		}

		// Token: 0x17000333 RID: 819
		// (get) Token: 0x0600121F RID: 4639 RVA: 0x00045045 File Offset: 0x00043245
		public string DisplayName
		{
			get
			{
				return this.displayName.ToPlainText();
			}
		}

		// Token: 0x17000334 RID: 820
		// (get) Token: 0x06001220 RID: 4640 RVA: 0x00045054 File Offset: 0x00043254
		public string Description
		{
			get
			{
				StringBuilder stringBuilder = new StringBuilder();
				string text = this.description.ToPlainText();
				if (!string.IsNullOrEmpty(text))
				{
					stringBuilder.AppendLine(text);
				}
				PerkBehaviour[] components = base.GetComponents<PerkBehaviour>();
				for (int i = 0; i < components.Length; i++)
				{
					string description = components[i].Description;
					if (!string.IsNullOrEmpty(description))
					{
						stringBuilder.AppendLine(description);
					}
				}
				return stringBuilder.ToString();
			}
		}

		// Token: 0x17000335 RID: 821
		// (get) Token: 0x06001221 RID: 4641 RVA: 0x000450BA File Offset: 0x000432BA
		public PerkRequirement Requirement
		{
			get
			{
				return this.requirement;
			}
		}

		// Token: 0x17000336 RID: 822
		// (get) Token: 0x06001222 RID: 4642 RVA: 0x000450C2 File Offset: 0x000432C2
		public bool DefaultUnlocked
		{
			get
			{
				return this.defaultUnlocked;
			}
		}

		// Token: 0x17000337 RID: 823
		// (get) Token: 0x06001223 RID: 4643 RVA: 0x000450CC File Offset: 0x000432CC
		private DateTime UnlockingBeginTime
		{
			get
			{
				DateTime dateTime = DateTime.FromBinary(this.unlockingBeginTimeRaw);
				if (dateTime > DateTime.UtcNow)
				{
					dateTime = DateTime.UtcNow;
					this.unlockingBeginTimeRaw = DateTime.UtcNow.ToBinary();
					GameManager.TimeTravelDetected();
				}
				return dateTime;
			}
		}

		// Token: 0x17000338 RID: 824
		// (get) Token: 0x06001224 RID: 4644 RVA: 0x00045111 File Offset: 0x00043311
		// (set) Token: 0x06001225 RID: 4645 RVA: 0x00045119 File Offset: 0x00043319
		public bool Unlocked
		{
			get
			{
				return this._unlocked;
			}
			internal set
			{
				this._unlocked = value;
				Action<Perk, bool> action = this.onUnlockStateChanged;
				if (action == null)
				{
					return;
				}
				action(this, value);
			}
		}

		// Token: 0x17000339 RID: 825
		// (get) Token: 0x06001226 RID: 4646 RVA: 0x00045134 File Offset: 0x00043334
		public bool Unlocking
		{
			get
			{
				return this.unlocking;
			}
		}

		// Token: 0x1700033A RID: 826
		// (get) Token: 0x06001227 RID: 4647 RVA: 0x0004513C File Offset: 0x0004333C
		// (set) Token: 0x06001228 RID: 4648 RVA: 0x00045144 File Offset: 0x00043344
		public PerkTree Master
		{
			get
			{
				return this.master;
			}
			internal set
			{
				this.master = value;
			}
		}

		// Token: 0x1700033B RID: 827
		// (get) Token: 0x06001229 RID: 4649 RVA: 0x0004514D File Offset: 0x0004334D
		public string DisplayNameRaw
		{
			get
			{
				return this.displayName;
			}
		}

		// Token: 0x1700033C RID: 828
		// (get) Token: 0x0600122A RID: 4650 RVA: 0x00045155 File Offset: 0x00043355
		public string DescriptionRaw
		{
			get
			{
				return this.description;
			}
		}

		// Token: 0x14000076 RID: 118
		// (add) Token: 0x0600122B RID: 4651 RVA: 0x00045160 File Offset: 0x00043360
		// (remove) Token: 0x0600122C RID: 4652 RVA: 0x00045198 File Offset: 0x00043398
		public event Action<Perk, bool> onUnlockStateChanged;

		// Token: 0x14000077 RID: 119
		// (add) Token: 0x0600122D RID: 4653 RVA: 0x000451D0 File Offset: 0x000433D0
		// (remove) Token: 0x0600122E RID: 4654 RVA: 0x00045204 File Offset: 0x00043404
		public static event Action<Perk> OnPerkUnlockConfirmed;

		// Token: 0x0600122F RID: 4655 RVA: 0x00045237 File Offset: 0x00043437
		public bool AreAllParentsUnlocked()
		{
			return this.master.AreAllParentsUnlocked(this);
		}

		// Token: 0x06001230 RID: 4656 RVA: 0x00045245 File Offset: 0x00043445
		private void OnValidate()
		{
			if (this.master == null)
			{
				this.master = base.GetComponentInParent<PerkTree>();
			}
		}

		// Token: 0x06001231 RID: 4657 RVA: 0x00045261 File Offset: 0x00043461
		private bool CheckAndPay()
		{
			return this.requirement == null || (EXPManager.Level >= this.requirement.level && this.requirement.cost.Pay(true, true));
		}

		// Token: 0x06001232 RID: 4658 RVA: 0x00045298 File Offset: 0x00043498
		public bool SubmitItemsAndBeginUnlocking()
		{
			if (this.Unlocked)
			{
				Debug.LogError("Perk " + this.displayName + " already unlocked!");
				return false;
			}
			if (!this.CheckAndPay())
			{
				return false;
			}
			this.unlocking = true;
			this.unlockingBeginTimeRaw = DateTime.UtcNow.ToBinary();
			this.master.NotifyChildStateChanged(this);
			Action<Perk, bool> action = this.onUnlockStateChanged;
			if (action != null)
			{
				action(this, this._unlocked);
			}
			return true;
		}

		// Token: 0x06001233 RID: 4659 RVA: 0x00045314 File Offset: 0x00043514
		public bool ConfirmUnlock()
		{
			if (this.Unlocked)
			{
				return false;
			}
			if (!this.unlocking)
			{
				return false;
			}
			if (DateTime.UtcNow - this.UnlockingBeginTime < this.requirement.RequireTime)
			{
				return false;
			}
			this.Unlocked = true;
			this.unlocking = false;
			this.master.NotifyChildStateChanged(this);
			Action<Perk> onPerkUnlockConfirmed = Perk.OnPerkUnlockConfirmed;
			if (onPerkUnlockConfirmed != null)
			{
				onPerkUnlockConfirmed(this);
			}
			return true;
		}

		// Token: 0x06001234 RID: 4660 RVA: 0x00045385 File Offset: 0x00043585
		public bool ForceUnlock()
		{
			if (this.Unlocked)
			{
				return false;
			}
			Debug.Log("Unlock default:" + this.displayName);
			this.Unlocked = true;
			this.unlocking = false;
			this.master.NotifyChildStateChanged(this);
			return true;
		}

		// Token: 0x06001235 RID: 4661 RVA: 0x000453C4 File Offset: 0x000435C4
		public TimeSpan GetRemainingTime()
		{
			if (this.Unlocked)
			{
				return TimeSpan.Zero;
			}
			if (!this.unlocking)
			{
				return TimeSpan.Zero;
			}
			TimeSpan timeSpan = DateTime.UtcNow - this.UnlockingBeginTime;
			TimeSpan timeSpan2 = this.requirement.RequireTime - timeSpan;
			if (timeSpan2 < TimeSpan.Zero)
			{
				return TimeSpan.Zero;
			}
			return timeSpan2;
		}

		// Token: 0x06001236 RID: 4662 RVA: 0x00045424 File Offset: 0x00043624
		public float GetProgress01()
		{
			TimeSpan remainingTime = this.GetRemainingTime();
			double totalSeconds = this.requirement.RequireTime.TotalSeconds;
			if (totalSeconds <= 0.0)
			{
				return 1f;
			}
			double totalSeconds2 = remainingTime.TotalSeconds;
			return 1f - (float)(totalSeconds2 / totalSeconds);
		}

		// Token: 0x06001237 RID: 4663 RVA: 0x00045470 File Offset: 0x00043670
		public void Validate(SelfValidationResult result)
		{
			if (this.master == null)
			{
				result.AddWarning("未指定PerkTree");
			}
			if (this.master)
			{
				if (!this.master.Perks.Contains(this))
				{
					result.AddError("PerkTree未包含此Perk").WithFix(delegate
					{
						this.master.perks.Add(this);
					}, true);
				}
				PerkTree perkTree = this.master;
				bool flag;
				if (perkTree == null)
				{
					flag = null != null;
				}
				else
				{
					PerkTreeRelationGraphOwner relationGraphOwner = perkTree.RelationGraphOwner;
					flag = ((relationGraphOwner != null) ? relationGraphOwner.GetRelatedNode(this) : null) != null;
				}
				if (!flag)
				{
					result.AddError("未在Graph中指定技能的关系");
				}
			}
		}

		// Token: 0x06001238 RID: 4664 RVA: 0x00045502 File Offset: 0x00043702
		internal Vector2 GetLayoutPosition()
		{
			if (this.master == null)
			{
				return Vector2.zero;
			}
			PerkTreeRelationGraphOwner relationGraphOwner = this.master.RelationGraphOwner;
			return ((relationGraphOwner != null) ? relationGraphOwner.GetRelatedNode(this) : null).cachedPosition;
		}

		// Token: 0x06001239 RID: 4665 RVA: 0x00045535 File Offset: 0x00043735
		internal void NotifyParentStateChanged()
		{
			Action<Perk, bool> action = this.onUnlockStateChanged;
			if (action == null)
			{
				return;
			}
			action(this, this.Unlocked);
		}

		// Token: 0x04000DEF RID: 3567
		[SerializeField]
		private PerkTree master;

		// Token: 0x04000DF0 RID: 3568
		[SerializeField]
		private bool lockInDemo;

		// Token: 0x04000DF1 RID: 3569
		[SerializeField]
		private Sprite icon;

		// Token: 0x04000DF2 RID: 3570
		[SerializeField]
		private DisplayQuality quality;

		// Token: 0x04000DF3 RID: 3571
		[LocalizationKey("Perks")]
		[SerializeField]
		private string displayName = "未命名技能";

		// Token: 0x04000DF4 RID: 3572
		[SerializeField]
		private bool hasDescription;

		// Token: 0x04000DF5 RID: 3573
		[SerializeField]
		private PerkRequirement requirement;

		// Token: 0x04000DF6 RID: 3574
		[SerializeField]
		private bool defaultUnlocked;

		// Token: 0x04000DF7 RID: 3575
		[SerializeField]
		internal bool unlocking;

		// Token: 0x04000DF8 RID: 3576
		[DateTime]
		[SerializeField]
		internal long unlockingBeginTimeRaw;

		// Token: 0x04000DF9 RID: 3577
		[SerializeField]
		private bool _unlocked;
	}
}
