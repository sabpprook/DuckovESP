...
public class CharacterRandomPreset : ScriptableObject
{
	...
	// (get) Token: 0x060004EA RID: 1258 RVA: 0x0001622F File Offset: 0x0001442F
	public string Name
	{
		get
		{
	...
	// (get) Token: 0x060004EB RID: 1259 RVA: 0x0001623C File Offset: 0x0001443C
	...
	// (get) Token: 0x060004EC RID: 1260 RVA: 0x00016249 File Offset: 0x00014449
	...
	// Token: 0x060004ED RID: 1261 RVA: 0x00016258 File Offset: 0x00014458
	...
	// Token: 0x060004EE RID: 1262 RVA: 0x000162CC File Offset: 0x000144CC
	...
	// Token: 0x060004EF RID: 1263 RVA: 0x0001633C File Offset: 0x0001453C
	private async UniTask<List<Item>> GenerateItems()
	{
		...
		if (global::UnityEngine.Random.Range(0f, 1f) < this.hasCashChance)
		{
			Item item = await ItemAssetsCollection.InstantiateAsync(GameplayDataSettings.ItemAssets.CashItemTypeID);
			item.StackCount = global::UnityEngine.Random.Range(this.cashRange.x, this.cashRange.y);
			items.Add(item);
		}
		return items;
	}

	// Token: 0x060004F0 RID: 1264 RVA: 0x00016380 File Offset: 0x00014580
	private async UniTask AddBullet(CharacterMainControl character)
	{
		Slot slot = character.PrimWeaponSlot();
		...
		if (item != null)
		{
			...
			{
				int random = this.bulletQualityDistribution.GetRandom(0f);
				this.bulletFilter.caliber = @string;
				this.bulletFilter.minQuality = random;
				this.bulletFilter.maxQuality = random;
				this.bulletFilter.excludeTags = this.bulletExclusiveTags;
				...
				if (array.Length >= 1)
				{
					...
					item2.StackCount = Mathf.RoundToInt((float)item2.StackCount * global::UnityEngine.Random.Range(this.bulletCountRange.x, this.bulletCountRange.y));
					if (character != null)
					{
	...
	// Token: 0x060004F3 RID: 1267 RVA: 0x00016610 File Offset: 0x00014810
	[CompilerGenerated]
