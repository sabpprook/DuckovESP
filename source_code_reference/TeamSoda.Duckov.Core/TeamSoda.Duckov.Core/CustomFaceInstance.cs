...
public class CustomFaceInstance : MonoBehaviour
{
	...
	{
		CustomFacePartUtility customFacePartUtility = null;
		CustomFacePartUtility customFacePartUtility2 = null;
		CustomFacePartCollection customFacePartCollection = null;
		switch (type)
		{
		case CustomFacePartTypes.hair:
			customFacePartUtility = this.hairPart;
			customFacePartCollection = this.data.Hairs;
			break;
		case CustomFacePartTypes.eye:
			customFacePartUtility = this.eyePart;
			customFacePartCollection = this.data.Eyes;
			break;
		case CustomFacePartTypes.eyebrow:
			customFacePartUtility = this.eyebrowPart;
			customFacePartCollection = this.data.Eyebrows;
			break;
		case CustomFacePartTypes.mouth:
			customFacePartUtility = this.mouthPart;
			customFacePartCollection = this.data.Mouths;
			break;
		case CustomFacePartTypes.tail:
			customFacePartUtility = this.tailPart;
			customFacePartCollection = this.data.Tails;
			break;
		case CustomFacePartTypes.foot:
			customFacePartUtility = this.footLPart;
			customFacePartUtility2 = this.footRPart;
			customFacePartCollection = this.data.Foots;
			break;
		case CustomFacePartTypes.wing:
			customFacePartUtility = this.wingLPart;
			customFacePartUtility2 = this.wingRPart;
			customFacePartCollection = this.data.Wings;
			break;
		}
		return new ValueTuple<CustomFacePartUtility, CustomFacePartUtility, CustomFacePartCollection>(customFacePartUtility, customFacePartUtility2, customFacePartCollection);
	}
	...
	public CustomFacePart SwitchPart(CustomFacePartTypes type, CustomFaceInstance parent, int direction)
	{
		...
		CustomFacePart customFacePart = this.SwitchOnePartInternal(item, nextOrPrevPrefab, parent, false);
		if (item2 != null)
		{
	...
	private CustomFacePart ChangePart(CustomFacePartTypes type, CustomFaceInstance parent, int id)
	{
		...
		CustomFacePart customFacePart = this.SwitchOnePartInternal(item, partPrefab, parent, false);
		if (item2 != null)
		{
	...
	private CustomFacePart InstantiatePartFromPrefab(CustomFacePart pfb)
	{
		...
		CustomFacePart customFacePart = null;
		if (Application.isPlaying)
		{
			customFacePart = global::UnityEngine.Object.Instantiate<CustomFacePart>(pfb);
		}
		return customFacePart;
	}
