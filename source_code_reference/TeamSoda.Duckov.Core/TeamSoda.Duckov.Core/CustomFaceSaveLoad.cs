...
public class CustomFaceSaveLoad : MonoBehaviour
{
	...
	private void Awake()
	{
		...
		for (int i = 0; i < 5; i++)
		{
			CustomFaceLoadSaveButton customFaceLoadSaveButton = this.CreateAButton(i, i.ToString(), this.slotButtonParent);
			this.slotButtons.Add(customFaceLoadSaveButton);
		}
	...
	{
		CustomFaceLoadSaveButton customFaceLoadSaveButton = global::UnityEngine.Object.Instantiate<CustomFaceLoadSaveButton>(this.buttonPfb);
	...
	private void LoadData(int slot)
	{
		...
		CustomFaceSettingData customFaceSettingData = (CustomFaceSettingData)ES3.Load(string.Format("CustomFaceData_{0}", slot));
		this.instance.LoadFromData(customFaceSettingData);
	}
	...
	{
		CustomFaceSettingData customFaceSettingData = this.instance.ConvertToSaveData();
		ES3.Save<CustomFaceSettingData>(string.Format("CustomFaceData_{0}", slot), customFaceSettingData);
	}
