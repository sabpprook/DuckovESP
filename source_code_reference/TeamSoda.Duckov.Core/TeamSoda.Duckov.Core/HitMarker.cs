...
public class HitMarker : MonoBehaviour
{
	...
	// (add) Token: 0x06000495 RID: 1173 RVA: 0x00014E68 File Offset: 0x00013068
	// (remove) Token: 0x06000496 RID: 1174 RVA: 0x00014E9C File Offset: 0x0001309C
	public static event Action OnHitMarker;

	// Token: 0x14000021 RID: 33
	// (add) Token: 0x06000497 RID: 1175 RVA: 0x00014ED0 File Offset: 0x000130D0
	// (remove) Token: 0x06000498 RID: 1176 RVA: 0x00014F04 File Offset: 0x00013104
	public static event Action OnKillMarker;

	// Token: 0x170000FA RID: 250
	// (get) Token: 0x06000499 RID: 1177 RVA: 0x00014F38 File Offset: 0x00013138
	...
	// Token: 0x0600049A RID: 1178 RVA: 0x00014F90 File Offset: 0x00013190
	private void Awake()
	{
	...
	// Token: 0x0600049B RID: 1179 RVA: 0x00014FE4 File Offset: 0x000131E4
	...
	// Token: 0x0600049C RID: 1180 RVA: 0x00015035 File Offset: 0x00013235
	...
	// Token: 0x0600049D RID: 1181 RVA: 0x00015058 File Offset: 0x00013258
	private void OnHit(DamageInfo dmgInfo)
	{
		...
		Vector3 vector = this.MainCam.WorldToScreenPoint(dmgInfo.damagePoint);
		Vector2 vector2;
		RectTransformUtility.ScreenPointToLocalPointInRectangle(base.transform.parent as RectTransform, vector, null, out vector2);
		base.transform.localPosition = Vector3.ClampMagnitude(vector2, 10f);
		ItemAgent_Gun gun = CharacterMainControl.Main.GetGun();
		if (gun != null)
		{
			this.scatterOnHit = gun.CurrentScatter;
		}
		int num = (flag ? (this.hitMarkerIndex ? this.critHash1 : this.critHash2) : (this.hitMarkerIndex ? this.hitHash1 : this.hitHash2));
		...
		if (shortNameHash != this.killHash && shortNameHash != this.killCritHash)
		{
			...
			this.animator.CrossFade(num, 0.02f);
		}
	...
	// Token: 0x0600049E RID: 1182 RVA: 0x000151D5 File Offset: 0x000133D5
	private void OnHealthKillEvent(Health _health, DamageInfo dmgInfo)
	{
		this.OnKill(dmgInfo);
	}

	// Token: 0x0600049F RID: 1183 RVA: 0x000151E0 File Offset: 0x000133E0
	private void OnKill(DamageInfo dmgInfo)
	{
		...
		bool flag = (float)dmgInfo.crit > 0f;
		int num = (flag ? this.killCritHash : this.killHash);
		this.animator.CrossFade(num, 0.02f);
		if (!dmgInfo.toDamageReceiver || !dmgInfo.toDamageReceiver.useSimpleHealth)
		{
	...
	// Token: 0x060004A0 RID: 1184 RVA: 0x00015295 File Offset: 0x00013495
	...
	// Token: 0x060004A1 RID: 1185 RVA: 0x000152AC File Offset: 0x000134AC
	private void OnSimpleHealthKill(HealthSimpleBase health, DamageInfo dmgInfo)
	{
		this.OnKill(dmgInfo);
	}

	// Token: 0x060004A2 RID: 1186 RVA: 0x000152B8 File Offset: 0x000134B8
